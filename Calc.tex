...
%\iffalse %IMPORTANT COMMENT
	...
	Множину можна конструювати як набір десяткових дробів, що не мають періоду. Наприклад, число $\sqrt{2} = 1.41421356237\dots$ Там же можна визначити всі операції - я цього робити не буду, вони будуть як аксіомами.
...
\end{definition}
	...
	Ця нерівність каже, що існує такий номер $n \in \mathbb{N}$, щоб взяти елемент $a \in A$, для якого, $a > c$. Але якщо точніше, ми можемо взяти $n = \left[ \dfrac{1}{2-2c} \right] + 1$, проте про цілу частину треба ще згодом поговорити.
	\bigskip \\
...
\end{example}
	...
	\begin{proposition}
	Якщо для кожної підпослідовність $\{a_{n_k}, k \geq 1\}: \exists \displaystyle\lim_{k \to \infty} a_{n_k} = a$, то для послідовності $\{a_n, n \geq 1\}: \exists \displaystyle\lim_{n \to \infty} a_n = a$
	\end{proposition}
	...
	$\displaystyle\lim_{k \to \infty} \left( 2 + \dfrac{3}{2k-1} \right) = 2$.\\
	Якщо $n = 2k$, то маємо підпослідовність $\left\{a_{n_k} = -2 - \dfrac{3}{2k}\right\}$.\\
	$\displaystyle\lim_{k \to \infty} \left(-2 - \dfrac{3}{2k} \right) = -2$.\\
	...
	Множина часткових границь: $X = \{-2, 2\}$. Тоді за означенням верхньої та нижньої границі, \\ $\displaystyle \uplim_{n \to \infty} a_n = 2, \displaystyle \downlim_{n \to \infty} a_n = -2$.\\
	Зауважимо одразу, що $\displaystyle \sup_{n \geq 1} \{a_n\} = 5$ та $\displaystyle \inf_{n \geq 1} \{a_n\} = -3.5$.
	\begin{figure}[H]
	\centering
	\begin{tikzpicture}[scale = 1.5]
	\centering
	...
	\end{tikzpicture}
	\end{figure}
	А якщо більш строго, то має бути так:\\
...
\end{figure}
	...
	\begin{example}
	Маємо функцію $f: \left[ -\dfrac{\pi}{2}, \dfrac{\pi}{2} \right] \to [-1,1]$, де $f(x) = \sin x$.\\
...
\end{figure}
Множину $f(X)$, тобто образ всієї області визначення, ще називають \textbf{областю значень}.\\
Позначення: $E(f)$.
\end{definition}
	...
	\begin{example}
	Задано функцію $f: \mathbb{R} \setminus \{2\} \to \mathbb{R}$, $f(x) = \dfrac{x^3-2x^2}{x-2}$. Довести, що $\displaystyle \lim_{x \to 2} \dfrac{x^3-2x^2}{x-2} = 4$.\\
...
1) $\displaystyle \lim_{x \to 0} \frac{\tg x}{x} = 1$\\
2) $\displaystyle \lim_{x \to 0} \frac{\arcsin x}{x} = 1$\\
3) $\displaystyle \lim_{x \to 0} \frac{\arctg x}{x} = 1$
\end{corollary}
...
Проблема в т. $x_0 = -1$. Але принаймні по одну сторону, наприклад $\displaystyle \lim_{x \to -1^+0} =\dfrac{1}{x+1} = +\infty$, матимемо нескінченність.\\
...
Отже, за Гейне та неперервністю, $\displaystyle \exists \lim_{n \to \infty} f(x_n) = f(x_0) \leq 0$.
\\
...
\begin{proof}
1.Розпишемо $\tg x = \dfrac{\sin x}{\cos x}$. Оскільки $\sin x \in \mathbb{R}$, $\cos x \in \mathbb{R}$, то врахуючи умову $\cos x \neq 0 \\ \implies x \neq \dfrac{\pi}{2} + \pi k, k \in \mathbb{Z}$, маємо $\tg x \in C\left(\mathbb{R} \setminus \left\{\dfrac{\pi}{2} + \pi k, k \in \mathbb{Z}\right\} \right)$ як частка.\\
2. Для $\ctg x$ аналогічні міркування.
\end{proof}

6) Задано функцію $f(x) = \arcsin x$. $f \in C([-1,1])$.
\begin{proof}
...
Отже, за теоремою про існування оберненої функції, $g^{-1}(x) = f(x) = \arcsin x \in C([-1,1])$.
\end{proof}
7) Задано функцію $f(x) = \arccos x$. $f \in C([-1,1])$.\\
\textit{Вказівка: $\arcsin x + \arccos x = \dfrac{\pi}{2}$.}
\bigskip \\
8) Задано функцію $f(x) = \arctg x$. $f \in C(\mathbb{R})$.
\begin{proof}
Маємо функцію $g(x) = \tg x$, що визначена на $\left( -\dfrac{\pi}{2}, \dfrac{\pi}{2} \right)$. На цьому проміжку вона монотонно строго зростає, неперервна.\\
Отже, за теоремою про існування оберненої функції, $g^{-1}(x) = f(x) = \arctg x \in C(\mathbb{R})$.
\end{proof}

9) Задано функцію $f(x) = \arcctg x$. $f \in C(\mathbb{R})$.\\
...
\begin{theorem}[Аксіома неперервності]
Нехай є дві множини $X,Y \subset \mathbb{R}$. Відомо, що $\forall x \in X$, $\forall y \in Y: x \leq y$. Тоді $\exists z \in \mathbb{R}: x \leq z \leq y$
\end{theorem}	

\begin{proof}
Покладемо $z = \sup X$ та доведемо, що $x \leq z \leq y$\\
Дійсно, за означенням, $\forall x \in X: x \leq z$\\
А число $y \in Y$ - верхня межа, але не найменша, тому $\forall y \in Y: z \leq y$
\end{proof}
\fi
\end{document}